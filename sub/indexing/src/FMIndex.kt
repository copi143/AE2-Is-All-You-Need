 ...
 *
 * 符号约定按 codec 类型区分：
 * - 字节路径（utf8，8bit 直存）：`0` 为唯一终结符（最末位），`0xFF` 为文档分隔符，
 *   普通字节为 `0x01~0xFE`，查询模式直接使用原始字节，不做重映射；
 * - 整数路径（utf16/utf32）：真实符号 `s -> s + 2`，文档分隔符 `-> 1`，唯一终结符 `-> 0`。
 * 查询模式只含真实符号，绝不会命中分隔符/终结符，故不会跨文档匹配。
 *
...
class FMIndex private constructor(
    ...
    fun search(pattern: String): List<Match> {
        return when (codec) {
            is ByteCodec -> {
                val pat = codec.encodeBytes(pattern)
    ...
    fun searchPrefix(prefix: String): List<Match> {
        return when (codec) {
            is ByteCodec -> {
                val raw = codec.encodeBytes(prefix)
                if (raw.isEmpty()) return emptyList()
                val pat = ByteArray(raw.size + 1) { i ->
                    if (i == 0) codec.separator.toByte() else raw[i - 1]
                }
    ...
    fun searchSuffix(suffix: String): List<Match> {
        return when (codec) {
            is ByteCodec -> {
                val raw = codec.encodeBytes(suffix)
                if (raw.isEmpty()) return emptyList()
                val pat = ByteArray(raw.size + 1) { i ->
                    if (i == raw.size) codec.separator.toByte() else raw[i]
                }
                core.locate(pat).map { mapContent(it, raw.size) }
            }
            is TextCodec -> {
    ...
    fun count(pattern: String): Int {
        return when (codec) {
            ...
            is TextCodec -> {
                ...
                core.count(IntArray(raw.size) { raw[it] + 2 })
            }
        }
    }
    ...
    companion object {
        fun build(texts: List<String>, codec: Codec = Utf8Codec): FMIndex {
            return when (codec) {
                is ByteCodec -> buildBytes(texts, codec)
                is TextCodec -> buildInts(texts, codec)
            }
        }

        private fun buildBytes(texts: List<String>, codec: ByteCodec): FMIndex {
            val alphabetSize = 256
            val encoded = texts.map { codec.encodeBytes(it) }
            var total = 1
            for (raw in encoded) total += raw.size + 2
            val text = ByteArray(total)
            val docContentStart = IntArray(texts.size)
            val docLengths = IntArray(texts.size)
            var pos = 0
            for (i in texts.indices) {
                val raw = encoded[i]
                text[pos++] = codec.separator.toByte()
                docContentStart[i] = pos
                raw.copyInto(text, pos)
                pos += raw.size
                docLengths[i] = raw.size
                text[pos++] = codec.separator.toByte()
            }
            text[pos] = 0
            val core = FmCore.build(text, alphabetSize)
            return FMIndex(core, codec, docContentStart, docLengths)
        }

        private fun buildInts(texts: List<String>, codec: TextCodec): FMIndex {
