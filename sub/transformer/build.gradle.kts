...
val r8 = configurations.create("r8").withDependencies {
    add(libs.r8.get())
}
...
dependencies {
    listOf(
        ...
        libs.asm.tree,
        libs.asm.analysis,
    ).forEach { compileOnly(it); r8Lib(it); testImplementation(it) }
    listOf(
        "cpw.mods:modlauncher:10.0.9",
        "net.minecraftforge:forgespi:7.0.1",
    ).forEach { compileOnly(it); r8Lib(it) }
    listOf(
        libs.fml,
    ).forEach { compileOnly(it) { isTransitive = false }; r8Lib(it) { isTransitive = false } }
