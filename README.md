...
- [x] `添加` 所有存储元件的 $1 \text{Ki}$ 到 $256 \text{Ti}$ 版本：物品、流体、魔力、能量、生命、饱食、经验
...
以 AE 能源为锚：$\text{AM} \equiv \text{AE} \equiv 2\ \text{FE}$。外部能量按其官方 X↔FE 转换率折算：

| 系统             | 官方转换依据                                | 汇率                                        |
|------------------|---------------------------------------------|---------------------------------------------|
| AM（本模组基准） | —                                           | $1 \text{AM} = 1 \text{AE} = 2 \text{FE}$   |
| Botania Mana     | 魔力磁场固定 $10 \text{FE} = 1 \text{Mana}$ | $1 \text{Mana} = 5 \text{AM} = 5 \text{AE}$ |
| Blood Magic LP   | TODO                                        | —                                           |
| Ars Nouveau Mana | TODO                                        | —                                           |

#### 存储元件

- [x] `魔力存储元件` $1 \text{Ki}$ ~ $256 \text{Ti}$，每字节存 $64 \text{AM}$，与能源元件一致
- 支持分区/反相/均分/溢出毁灭卡，与物品元件一致

所有存储元件共用同一套实现，仅按键空间区分；每格可存类型数上限沿用 AE2 原版口径：
...
- [ ] 随机是不存在的，所有概率配方自动倍增为普通配方。

原始配方：$3 \text{A} \Rightarrow 2 \text{B}_{35\%}$

修改配方：$60 \text{A} \Rightarrow 14 \text{B}$

此处还要保证配方不能执行半次，即 $3$ 要为 $60$ 的因数，$2$ 要为 $14$ 的因数，故不会除以 $2$。

#### 打包多方块

- [ ] 让多方块结构能直接放进分子装配室内执行配方。
